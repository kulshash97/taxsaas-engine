...
def parse_universal_bank_statement(file_buffered):
    """
    Surgically computes precise cumulative credit inflows locally without API tokens.
    ...
    try:
        ...
        for line in lines:
            ...
            if any(meta in line_clean.lower() for meta in ["page", "opening balance", "closing balance", "date range"]):
                continue
...
def parse_income_tax_ais(file_buffered):
    """
    Scans and reads high-value investments locally directly from AIS uploads.
    """
...
with panel_left:
    ...
    uploaded_statement = st.file_uploader("Upload Bank Statement (PDF ONLY)", type=["pdf"], key="bank_prod_fixed")
    ...
    uploaded_ais = st.file_uploader("Upload Client Annual Information Statement (AIS) PDF", type=["pdf"], key="ais_prod_fixed")
    if uploaded_ais:
