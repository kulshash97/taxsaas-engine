...
def call_gemini(system_prompt: str, user_prompt: str, max_tokens: int = 1500) -> str:
    ...
    - Model fallback chain: gemini-2.0-flash → gemini-1.5-flash-latest → gemini-1.0-pro
    - Retry with 30s wait on 429 (quota exhausted)
    - Clear user-facing messages for quota vs auth errors
    - Token budget kept lean (max_tokens capped at 1500 by default)
    """
    GEMINI_API_KEY = st.secrets.get("GEMINI_API_KEY", os.environ.get("GEMINI_API_KEY",""))
    if not GEMINI_API_KEY:
        return "❌ GEMINI_API_KEY not configured. Go to Streamlit Cloud → Settings → Secrets and add:\nGEMINI_API_KEY = \"your-key-here\""
    ...
    for model in models:
        ...
        for attempt in range(2):
            try:
                ...
                if e.code == 429:
                    if attempt == 0:
                        # Wait 30 s then retry same model once
                        time.sleep(30)
                        continue
                    else:
                    ...
                    return ("❌ **API Key Error (403):** Your Gemini API key is invalid or "
                            "does not have permission.\n\n"
                            "**Fix:** Go to [aistudio.google.com](https://aistudio.google.com) "
                            "→ Get API Key → copy fresh key → paste in Streamlit Secrets.")
                else:
    ...
    # All models exhausted — 429 quota issue
    return (
        "⚠️ **Gemini Free Tier Quota Exhausted**\n\n"
        "Your free API key has hit its daily/minute limit. This is a Google account issue, not a code bug.\n\n"
        "**Fix Option A — Enable Billing (Recommended, takes 5 min):**\n"
        "1. Go to [console.cloud.google.com](https://console.cloud.google.com)\n"
        "2. Select your project → Billing → Link a billing account\n"
        "3. Your same API key starts working immediately\n"
        "4. Cost is ~₹0.006 per 1,000 tokens — a full tax report costs under ₹0.50\n\n"
        "**Fix Option B — Wait:**\n"
        "Free tier resets every 24 hours. Try again tomorrow.\n\n"
        "**Fix Option C — New Free Key:**\n"
        "Create a new Google Cloud project at [aistudio.google.com](https://aistudio.google.com) "
        "→ Get API Key → update Streamlit Secrets.\n\n"
        "The ITR computation and PDF generation work perfectly without the AI. "
        "Download your compliance reports using the buttons below."
    )
