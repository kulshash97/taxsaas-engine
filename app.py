...
#  Groq: FREE, no billing, 6000 RPD, fast
...
def _call_gemini(system_prompt: str, user_prompt: str, max_tokens: int = 1500) -> str:
    ...
    payload  = json.dumps({
        ...
        "generationConfig": {"maxOutputTokens": min(max_tokens, 1500), "temperature": 0.3}
    }).encode()
...
def generate_itr_pdf(name, pan, firm, result, report_type="Standard Compliance"):
    ...
    meta = [
        [Paragraph(f"<b>Assessee:</b> {name}", body_s),
         Paragraph(f"<b>Assessment Year:</b> 2026-27 (FY 2025-26)", body_s)],
    ...
    rows = [[Paragraph("<b>Field</b>", body_s),
             Paragraph("<b>Amount (INR)</b>", body_s)]]
    for k, v in result["metrics"].items():
        rows.append([Paragraph(k, body_s), Paragraph(_r(v), body_s)])
    ...
    rows2 = [[Paragraph("<b>Component</b>", body_s),
              Paragraph("<b>Amount (INR)</b>", body_s)]]
    ...
    gr      = result['metrics']['Gross Receipts / Turnover']
    pp      = result['metrics']['Presumptive Profit (44AD/44ADA)']
    stcg    = result['metrics']['STCG -- Sec 111A (Listed Equity 20%)'] \
              if 'STCG -- Sec 111A (Listed Equity 20%)' in result['metrics'] \
              else result['metrics'].get('STCG - Sec 111A (Listed Equity 20%)', 0)
    # safe key lookup
    stcg_key = [k for k in result['metrics'] if '111A' in k]
    ltcg_key = [k for k in result['metrics'] if '112A' in k]
    stcg_v   = result['metrics'][stcg_key[0]] if stcg_key else 0
    ltcg_v   = result['metrics'][ltcg_key[0]] if ltcg_key else 0

    steps = [
        f"<b>Step 1 - Form Selection:</b> Login to incometax.gov.in → File ITR → AY 2026-27 → Select <b>{result['assigned_form']}</b>.",
        f"<b>Step 2 - Regime:</b> Select <b>{result['regime']} REGIME</b> under Section 115BAC. Confirm before proceeding.",
        f"<b>Step 3 - Schedule BP:</b> Enter Gross Receipts: <b>{_r(gr)}</b> | Presumptive Profit: <b>{_r(pp)}</b>",
        f"<b>Step 4 - Schedule CG:</b> Sec 111A STCG: <b>{_r(stcg_v)}</b> @ 20%  |  Sec 112A LTCG: <b>{_r(ltcg_v)}</b> @ 12.5%",
        f"<b>Step 5 - Part B-TTI:</b> Verify Sec 87A Rebate: <b>{_r(rebate)}</b>  |  Net Tax Payable: <b>{_r(net_tax)}</b>",
        "<b>Step 6 - Pre-Submit:</b> Cross-verify all TDS credits with Form 26AS and AIS. Match every entry.",
        "<b>Step 7 - E-Verify:</b> Submit → Preview → E-Verify via Aadhaar OTP or Net Banking within 30 days of filing.",
    ]
    for s in steps:
    ...
    story.append(Paragraph(
        ...
        "All figures must be verified with original source documents before filing. "
        "KSP is not liable for any errors arising from incorrect data inputs.",
        small_s))
    doc.build(story)
    buf.seek(0)
    return buf.getvalue()
...
def generate_gst_pdf(biz_name, gstin, firm, result):
    ...
    meta = [
        ...
        [Paragraph(f"<b>Turnover:</b> {_r(result['annual_turnover'])}", body_s),
    ...
    rows = [[Paragraph("<b>Component</b>", body_s),
             Paragraph("<b>Amount (INR)</b>", body_s)]]
